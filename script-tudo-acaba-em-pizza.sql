...
BEGIN
	insert into item_pedido values (0, (select valor_unit from produtos where id = spproduto_id),spquantidade, sppedido_id, spproduto_id,sptamanho_pizza_id);
END$$
...
BEGIN
	delete from item_pedio where id = spid;
END$$
...
BEGIN
	update produtos set nome = spnome,descricao = spdescricao, valor_unit = spvalor_unit, cod_barras = spcod_barras, link_imagem = splink_imagem, categorias_id = spcategorias_id where id = spid;
END$$
...
BEGIN
	update usuarios set nome = spnome, senha = aes_encrypt(spsenha,'NewTech@123'), niveis_id = spniveis_id where id = spid;
END$$
...
BEGIN
	insert into pagamento values (0,sptipo_pagamento_id,spparcelas);
END$$
...
BEGIN
	update pagamento set tipo_pagamento = sptipo_pagamento_id where id = spid;
END$$
...
-- -----------------------------------------------------
-- procedure sp_pedidoadicional_insert
-- -----------------------------------------------------
...
BEGIN
	insert into pedidoadicional values(0,sppedido_id,spadicionais_id);
END$$
...
-- -----------------------------------------------------
-- procedure sp_pedidotipoborda_insert
-- -----------------------------------------------------
...
BEGIN
	insert into sp_pedido_tipo_borda values (0,sppedido_id,sptipoborda_id);
END$$
...
BEGIN
	insert into tamanho_pizza values (0,spsigla,spdescricao,default);
END$$
...
BEGIN
	update tamanho_pizza set sigla = spsigla, descricao = spdescricao where id = spid;
END$$
...
BEGIN
	update tamanho_pizza set ativo = spativo where id = spid;
END$$
